...
function createServer(isDev) {
  ...
  if (isDev) {
    ...
    databasePath = path
      .join(__dirname, "/Database/Dave.db")
  ...
  app.post("/CreateProduct", async (req, res) => {
    ...
    if ((body.nombre !== "" && body.precio !== "", body.id_categoria !== "")) {
      db.run("INSERT INTO PRODUCTO(nombre,precio,id_categoria) VALUES(?,?,?)", [
        body.nombre,
        body.precio,
        body.id_categoria,
      ])
...
databasePath = path.join(__dirname, "/Database/Dave.db").replace("app.asar", "app.asar.unpacked");
